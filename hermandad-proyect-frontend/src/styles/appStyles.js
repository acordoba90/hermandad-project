...
export const appStyles = {
  ...
  characterOnboarding: {
    ...
    candidateCard: { height: '100%', overflow: 'hidden', borderRadius: '24px', display: 'flex', flexDirection: 'column', transition: 'transform 200ms ease, border-color 200ms ease, box-shadow 200ms ease', '&:hover': { transform: 'translateY(-4px)', borderColor: 'rgba(212,175,55,0.5)' }, '@media (prefers-reduced-motion: reduce)': { transition: 'none', '&:hover': { transform: 'none' } } },
    ...
    candidatePortrait: { position: 'relative', minHeight: 190, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(180deg, rgba(75,29,92,0.58), rgba(14,10,18,0.96))', borderBottom: '1px solid', borderColor: 'divider' },
    candidateType: { position: 'absolute', top: 16, left: 16, color: 'secondary.light', backgroundColor: 'rgba(14,10,18,0.82)', border: '1px solid rgba(212,175,55,0.35)' },
    selectedBadge: { position: 'absolute', top: 16, right: 16, display: 'flex', alignItems: 'center', gap: 0.5, px: 1.25, py: 0.6, borderRadius: 999, color: 'secondary.contrastText', backgroundColor: 'secondary.main' },
    candidateContent: { p: 3, display: 'flex', flexDirection: 'column', gap: 2.25, flex: 1, '&:last-child': { pb: 3 }, '& > button': { mt: 'auto' } },
    traits: { display: 'flex', flexWrap: 'wrap', gap: 1 },
    ...
    traitHeading: { mb: 2.5, display: 'flex', alignItems: { xs: 'flex-start', sm: 'center' }, justifyContent: 'space-between', gap: 2, flexDirection: { xs: 'column', sm: 'row' } },
  },
};
